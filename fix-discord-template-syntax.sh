...
# Updates template syntax to proper n8n expressions
...
SQL_FILE="/tmp/fix_discord_template_$$.sql"
...
**Timestamp**: " || (CURRENT_TIMESTAMP AT TIME ZONE ''UTC'')::text || "
**Status**: ✅ Success

🤖 *Manual Blog Automation*
📅 " || (CURRENT_TIMESTAMP AT TIME ZONE ''Australia/Sydney'')::text || ""'
)
...
scp "$SQL_FILE" "$SSH_HOST:/tmp/fix_discord_template.sql"

# Execute the SQL on VPS
ssh "$SSH_HOST" "sudo -u postgres psql -d n8n -f /tmp/fix_discord_template.sql"

# Clean up
rm -f "$SQL_FILE"
ssh "$SSH_HOST" "rm -f /tmp/fix_discord_template.sql"

echo -e "${GREEN}✅ Discord notification template syntax fixed successfully${NC}"
echo -e "${YELLOW}📝 Success notification now shows actual values instead of template syntax${NC}"
echo -e "${YELLOW}📝 Error notification now shows actual values instead of template syntax${NC}"