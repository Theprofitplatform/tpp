...
function m({ href: e }) {
  try {
    const t = new URL(e);
    return (
      window.location.origin === t.origin && window.location.pathname !== t.pathname && !v.has(e)
    );
  } catch {}
  return !1;
}
let p, c;
function E(e) {
  (v.add(e.href), c.observe(e), l.map(t => e.addEventListener(t, h, { passive: !0, once: !0 })));
}
function L(e) {
...
async function w(e) {
  ...
  try {
    ...
    await Promise.all(s.filter(n => !d.has(n.href)).map(n => (d.add(n.href), fetch(n.href))));
  } catch {}
}
function k({
  ...
  if ('connection' in navigator) {
    ...
    if (n.saveData) return Promise.reject(new Error('Cannot prefetch, Save-Data is enabled'));
  ...
  ((c =
    ...
    new IntersectionObserver(n => {
      n.forEach(o => {
        if (o.isIntersecting && o.target instanceof HTMLAnchorElement) {
          ...
          let i = !1;
          (Array.isArray(r) ? (i = r.some(u => f.includes(u))) : (i = f.includes(r)),
    ...
    b(() => {
      ...
      [...document.querySelectorAll(o)].filter(m).forEach(i => {
        l.map(u => i.addEventListener(u, h, { passive: !0, once: !0 }));
      });
    }));
}
k({ selector: 'a[href^="/"]', throttle: 3 });
